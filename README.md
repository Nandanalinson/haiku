# 🌸 Haiku Generator

*A word-powered haiku generator built with Flask, HTML, CSS, JavaScript, and the Gemini API.*

## ✨ Overview

Haiku Generator is a simple web app where the user enters **one word**, and the backend creates an original **haiku poem** based on that word.
The project combines:

* **Flask** – backend server
* **HTML / CSS / JavaScript** – frontend UI
* **Gemini API** – AI that generates haiku responses

This project is lightweight, beginner-friendly, and easy to deploy on services like Render, Railway, or Heroku.

---

## 🖼️ Features

* 🌸 Enter any word and receive a **3-line haiku**
* ⚡ Instant API-driven responses
* 🎨 Simple, responsive UI (HTML/CSS/JS)
* 🔧 Flask backend with clean API routes
* 🧠 Uses Gemini API for haiku generation

---

## 🗂️ Project Structure

```
/
├── static/
│   ├── style.css
│   └── script.js
├── templates/
│   └── index.html
├── main.py
├── requirements.txt
└── README.md
```

---

## 🧩 Technologies Used

* **Python 3**
* **Flask**
* **HTML5**
* **CSS3**
* **JavaScript**
* Optional deployment:

